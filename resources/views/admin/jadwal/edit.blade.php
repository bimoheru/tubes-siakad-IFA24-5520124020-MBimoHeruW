            ...
            <form method="POST" action="{{ route('admin.jadwal.update', $jadwal->id) }}">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem">
                    <div class="form-group">
                        ...
                        <input id="kelas" type="text" name="kelas" maxlength="5"
                            class="form-control @error('kelas') is-invalid @enderror"
                            value="{{ old('kelas', $jadwal->kelas) }}" placeholder="A / B / C / 1.1 / 1.2" required>
                        @error('kelas')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem">
                    ...
                    <div class="form-group">
                        ...
                        <input id="jam_selesai" type="time" name="jam_selesai"
                            ...
                            value="{{ old('jam_selesai', $jadwal->jam_selesai?->format('H:i')) }}">
                        @error('jam_selesai')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div style="display:flex; gap:.75rem; margin-top:1.5rem">
                    ...
                    <a href="{{ route('admin.jadwal.index') }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i>
                        Batal</a>
                </div>
            </form>
        </div>
